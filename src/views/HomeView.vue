  ...
  <main class="page home-page">
    ...
    <section v-if="friendlyLists.length" class="roster-list-group" aria-label="Army rosters">
      ...
      <div class="saved-list-stack"><article v-for="list in friendlyLists" :key="list.id" class="saved-list-card card-surface" :class="`roster-status-${rosterState(list)}`">
        <RouterLink :to="rosterOpenRoute(list)" class="saved-list-open-area"><div><strong class="saved-list-name-points"><span>{{ list.name }}</span><small>— {{ actualPoints(list) }} / {{ list.points }} pts</small></strong><div class="saved-list-labels"><span v-if="isLegacyArmy(list.army)" class="app-option-label legacy-roster-label">LEGACY</span><span class="app-option-label">{{ list.armyName }}</span><span class="app-option-label">{{ list.compositionName }}</span><span v-for="option in list.options" :key="`${list.id}-${option}`" class="app-option-label composition-selected-label">{{ optionLabel(option) }}</span></div></div></RouterLink>
        <div class="saved-list-row-actions">
          <RouterLink class="saved-list-icon-action" :to="rosterOpenRoute(list)" aria-label="View roster" title="View"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.8 12s3.2-5 9.2-5 9.2 5 9.2 5-3.2 5-9.2 5-9.2-5-9.2-5Z"/><circle cx="12" cy="12" r="2.5"/></svg></RouterLink>
    ...
    <section v-if="enemyLists.length" class="roster-list-group enemy-roster-group" aria-label="Enemy army rosters">
      ...
      <div class="saved-list-stack"><article v-for="list in enemyLists" :key="list.id" class="saved-list-card card-surface enemy-roster-card" :class="`roster-status-${rosterState(list)}`">
        <RouterLink :to="rosterOpenRoute(list)" class="saved-list-open-area"><div><strong class="saved-list-name-points"><span>{{ list.name }}</span><small>— {{ actualPoints(list) }} / {{ list.points }} pts</small></strong><div class="saved-list-labels"><span class="app-option-label enemy-roster-label">Enemy Roster</span><span v-if="isLegacyArmy(list.army)" class="app-option-label legacy-roster-label">LEGACY</span><span class="app-option-label">{{ list.armyName }}</span><span class="app-option-label">{{ list.compositionName }}</span><span v-for="option in list.options" :key="`${list.id}-${option}`" class="app-option-label composition-selected-label">{{ optionLabel(option) }}</span></div></div></RouterLink>
        <div class="saved-list-row-actions enemy-roster-view-actions">
          <RouterLink class="saved-list-icon-action" :to="rosterOpenRoute(list)" aria-label="View enemy roster" title="View"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.8 12s3.2-5 9.2-5 9.2 5 9.2 5-3.2 5-9.2 5-9.2-5-9.2-5Z"/><circle cx="12" cy="12" r="2.5"/></svg></RouterLink>
