...
function rosterState(list: SavedArmyList) {
  ...
  if (openMagicAllowancePoints(list) > 0 && !isMagicAllowanceApproved(list.id, list.roster || [])) return 'warning'
  ...
  <main class="page home-page">
    ...
    <section class="list-launch card-surface">
      ...
      <div v-if="!deleteMode" class="list-launch-actions">
        ...
        <button class="list-delete-mode-button" type="button" aria-label="Select rosters to delete" title="Delete rosters" @click="toggleDeleteMode"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h17"/><path d="M9 6.5V4h6v2.5"/><path d="m6.5 6.5.9 13h9.2l.9-13"/><path d="M10 10.5v5.5M14 10.5v5.5"/></svg></button>
      </div>
      ...
      <div class="saved-list-stack">
        <article v-for="list in enemyLists" :key="list.id" class="saved-list-card card-surface enemy-roster-card" :class="[{ 'delete-select-mode': deleteMode, selected: selectedForDelete.has(list.id) }, `roster-status-${rosterState(list)}`]">
          <RouterLink v-if="!deleteMode" :to="{ name: 'list-view', params: { listId: list.id } }" class="saved-list-open-area">
            <div><strong>{{ list.name }}</strong><div class="saved-list-labels"><span class="app-option-label enemy-roster-label">Enemy Roster</span><span v-if="isLegacyArmy(list.army)" class="app-option-label legacy-roster-label">LEGACY</span><span class="app-option-label">{{ list.armyName }}</span><span class="app-option-label">{{ list.compositionName }}</span><span v-for="option in list.options" :key="`${list.id}-${option}`" class="app-option-label composition-selected-label">{{ optionLabel(option) }}</span></div></div>
          ...
          <div v-if="!deleteMode" class="saved-list-row-actions enemy-roster-view-actions"><RouterLink class="saved-list-icon-action" :to="{ name: 'list-view', params: { listId: list.id } }" aria-label="View enemy roster" title="View enemy roster"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.8 12s3.2-5 9.2-5 9.2 5 9.2 5-3.2 5-9.2 5-9.2-5-9.2-5Z"/><circle cx="12" cy="12" r="2.5"/></svg></RouterLink><button type="button" class="saved-list-icon-action enemy-roster-toggle active" aria-label="Move enemy roster back to friendly rosters" title="Move to Army Rosters" @click="toggleEnemyRoster(list)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 21V4"/><path d="M6 5h11l-2.5 3L17 11H6"/></svg></button></div>
