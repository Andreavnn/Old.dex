...
const combatTopLevelUnits = computed(() => playerRoster.value)
...
function matchUnitProfileRoute(row: BuilderRosterSelection) {
  ...
  const params = new URLSearchParams({
    ...
    rule,
    options,
    points: String(game.value?.points || list?.points || 0),
    list: game.value?.playerListId || list?.id || '',
  })
  return {
    path: `/army/${armySlug}/unit/${row.unitId}`,
    query: { composition, rule, options, instance: row.instanceId, mode: 'view', return: `/lists/builder?${params.toString()}` },
  }
}
...
async function hydrateScenarioGuidance() {
  if (!game.value) return
  if (game.value.scenarioGuidance) {
    if (!game.value.roundLimitCustomized) {
      const desired = scenarioDefaultRounds(game.value.scenarioGuidance)
...
<template>
  <main class="page game-match-page">
    ...
    <section v-if="game" class="game-match-shell">
      ...
      <div v-if="phase" class="match-tab-scroll-shell step-scroll-shell" :class="{ 'single-step': phase.steps.length === 1 }"><button v-if="phase.steps.length > 1" type="button" class="match-tab-scroll-arrow left" aria-label="Scroll phase steps left" @click="scrollTabs('step', -1)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 5 8.5 12l7 7" /></svg></button><div ref="stepTabsRef" class="game-step-list game-subphase-tabs" aria-label="Current phase subphases"><button v-for="(item, index) in phase.steps" :key="item.id" type="button" :class="{ active: game.stepIndex === index }" :disabled="isReadOnly" @click="setStep(index)"><span>{{ index + 1 }}</span>{{ item.label }}</button></div><button v-if="phase.steps.length > 1" type="button" class="match-tab-scroll-arrow right" aria-label="Scroll phase steps right" @click="scrollTabs('step', 1)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m8.5 5 7 7-7 7" /></svg></button></div>
