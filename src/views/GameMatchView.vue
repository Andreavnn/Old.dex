...
import OngoingBattleConditionsPanel from '../components/OngoingBattleConditionsPanel.vue'
...
import { getSavedArmyList, getSavedArmyLists, type SavedArmyList } from '../services/savedLists'
import { hydrateFriendlyMagicSetup, loadMagicChoices, loadScenarioGuidance, randomHappeningOptions } from '../services/gameSetup'
...
import { deploymentSequenceLabel, deploymentSequenceRank, isDeploymentScout, isDeploymentWarMachine } from '../core/deploymentOrder'
...
const changeRosterSide = ref<GameSide | null>(null)
const changeRosterId = ref('')
...
const rosterChangeChoices = computed(() => {
  if (!changeRosterSide.value) return [] as SavedArmyList[]
  const rows = getSavedArmyLists()
  if (changeRosterSide.value === 'player') return rows.filter((row) => !row.enemyRoster)
  const enemy = rows.filter((row) => row.enemyRoster)
  return (enemy.length ? enemy : rows).filter((row) => row.id !== game.value?.playerListId)
})
...
const ongoingBattleConditionDisplayRows = computed(() => ongoingBattleConditions.value.map((condition) => ({
  id: condition.id,
  label: condition.label,
  path: condition.path,
  result: ongoingBattleConditionResult(condition),
})))
...
const compulsoryMovementUnits = computed(() => {
  ...
  for (const unit of playerRoster.value) if (persistentUnitState(unit.instanceId).pursuedOffTable) ids.add(unit.instanceId)
...
function isWarMachine(row: BuilderRosterSelection) { return isDeploymentWarMachine(row) }
function isScoutUnit(row: BuilderRosterSelection) { return isDeploymentScout(row) }
...
const deploymentRosterGroups = computed(() => [
  ...
  { id: 'war-machines', label: '2 · WAR MACHINES', detail: 'Resolve the War Machine deployment step, recording each unit separately', rows: deploymentRosterRows.value.filter((row) => deploymentSequenceRank(row) === 1) },
...
function matchRuleIdentity(rule: { label: string; path?: string }) {
  const label = String(rule.label || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()
  const path = String(rule.path || '').toLowerCase().replace(/[?#].*$/, '').replace(/\/+$/, '')
  // The same canonical rule can arrive from a joined Character and its host
  // with slightly different path metadata. The visible rule name is therefore
  // the stable identity; path is only a fallback when a label is unavailable.
  return label || path
}
function dedupeMatchRules<T extends { label: string; path?: string }>(rows: T[]) {
  const seen = new Set<string>()
  return rows.filter((row) => { const key = matchRuleIdentity(row); if (!key || seen.has(key)) return false; seen.add(key); return true })
}
function joinedCharacterChargeRules(hostId: string) {
  const ids = new Set(joinedCharactersForHost(hostId).map((row) => row.instanceId))
  return dedupeMatchRules(allDeclareChargeGuidance.value
...
function chargeRelatedRules(rule: MatchGuidanceRule) {
  ...
  return dedupeMatchRules([...(rule.relatedRules || []), ...joinedCharacterChargeRules(hostId)])
}
...
function setChargeSuccessful(instanceId: string, checked: boolean) {
  patchUnitTurnState(instanceId, { inCombat: false, chargeHeld: false, chargeDeclared: checked, chargeSuccessful: checked })
...
function setChargeReaction(instanceId: string, reaction: 'hold' | 'stand-shoot' | 'flee' | 'counter-charge', checked: boolean) {
  const previous = chargeReaction(instanceId)
  const penalties = shootingPenaltyIds(instanceId)
  if (checked && previous === 'stand-shoot' && reaction !== 'stand-shoot') penalties.delete('stand-shoot')
  if (reaction === 'stand-shoot') {
    if (checked) { penalties.add('stand-shoot'); penalties.delete('long-range') } else penalties.delete('stand-shoot')
  }
  patchUnitTurnState(instanceId, { chargeReaction: checked ? reaction : (previous === reaction ? '' : previous), shootingPenaltyIds: [...penalties] })
  if (checked && previous === 'flee' && reaction !== 'flee') setFleeing(instanceId, false)
...
function chargeReactionRules(instanceId: string) {
  ...
  return dedupeMatchRules(turnGuidance.value.filter((rule) => rule.intent === 'reaction' && rule.sourceKind !== 'core' && (rule.unitRefs || []).some((ref) => ids.has(ref.instanceId))))
}
function compulsoryMoved(instanceId: string) { return Boolean(unitTurnState(instanceId).compulsoryMoved) }
function setCompulsoryMoved(instanceId: string, checked: boolean) { patchUnitTurnState(instanceId, { compulsoryMoved: checked }); if (checked && persistentUnitState(instanceId).pursuedOffTable) patchPersistentUnitState(instanceId, { pursuedOffTable: false, pursuedOffTableSinceRound: undefined }) }
...
function setRemainingMoveMode(instanceId: string, mode: Exclude<RemainingMoveMode, ''>, checked: boolean) {
  ...
  patchUnitTurnState(instanceId, { remainingMoveMode: nextMode, remainingMoved: nextMode === 'normal' || nextMode === 'march' })
}
...
function unitMovedThisTurn(instanceId: string) {
  const state = unitTurnState(instanceId)
  return Boolean(state.chargeDeclared || state.chargeSuccessful || state.compulsoryMoved || state.remainingMoved || state.rallyResult === 'pass' || ['normal','march'].includes(String(state.remainingMoveMode || '')))
}
function weaponHasMoveOrShoot(weapon: NonNullable<MatchUnitProfileSnapshot>['weapons'][number]) { return (weapon.rules || []).some((rule) => /^Move or Shoot(?:\s|\(|$)/i.test(rule)) }
function shootingWeaponLocked(instanceId: string, weapon: NonNullable<MatchUnitProfileSnapshot>['weapons'][number]) { return unitMovedThisTurn(instanceId) && weaponHasMoveOrShoot(weapon) }
function shootingUnitBlocked(entry: { unit: BuilderRosterSelection; weapons: NonNullable<MatchUnitProfileSnapshot>['weapons'] }) { return entry.weapons.length > 0 && entry.weapons.every((weapon) => shootingWeaponLocked(entry.unit.instanceId, weapon)) }
const standShootSelectablePenalties = shootingPenaltyOptions.filter((row) => !['moving','long-range','stand-shoot'].includes(row.id))
...
function unitEliminated(unit: BuilderRosterSelection) { return Boolean(persistentUnitState(unit.instanceId).warMachineAbandoned) || destroyedCount(unit.instanceId) >= casualtyLimit(unit) }
...
function hasImpactHits(row: BuilderRosterSelection) {
  const ids = new Set([row.instanceId, ...joinedCharactersForHost(row.instanceId).map((entry) => entry.instanceId)])
  return playerRoster.value.filter((entry) => ids.has(entry.instanceId)).some((entry) => (entry.specialRules || []).some((rule) => /^Impact Hits(?:\s|\(|$)/i.test(rule.label)))
}
function pursuedOffTable(instanceId: string) { return Boolean(persistentUnitState(dispositionOwnerId(instanceId)).pursuedOffTable) }
function setPursuedOffTable(instanceId: string, checked: boolean) {
  const ownerId = dispositionOwnerId(instanceId)
  patchPersistentUnitState(ownerId, { pursuedOffTable: checked, pursuedOffTableSinceRound: checked ? game.value?.round : undefined })
  patchUnitTurnState(ownerId, { pursuedOffTable: checked })
  if (checked) recordHistory('combat', ownerId, 'pursued-off-table', "Returns during the controlling player's next Compulsory Moves subphase.")
}
function hasNotPaidToFight(row: BuilderRosterSelection) { return isWarMachine(row) || (row.specialRules || []).some((rule) => /^(?:We['’]?re|We Are) Not Paid to Fight(?:\s|\(|$)/i.test(rule.label)) }
function setWarMachineAbandoned(row: BuilderRosterSelection, checked: boolean) {
  patchPersistentUnitState(row.instanceId, { warMachineAbandoned: checked })
  if (checked) recordHistory('combat', row.instanceId, 'destroyed', "We're Not Paid to Fight")
}
...
async function ensureCombatProfile(row: BuilderRosterSelection) {
  if (!game.value || combatProfiles.value[row.instanceId] || combatProfileLoading.value.has(row.instanceId)) return
...
function scrollTabs(_target: 'phase', direction: -1 | 1) {
  const element = phaseTabsRef.value
  if (!element) return
...
function guidanceDisabled(rule: MatchGuidanceRule, _index: number) {
...
function setSpellResult(rule: MatchGuidanceRule, result: 'success' | 'fail' | '') {
  ...
  persist({ stepChecks: { ...(game.value.stepChecks || {}), [checklistKey.value]: group } })
  patchSpellCastState(rule, { result })
}
...
function wizardCompletedCastCount(caster: GameMagicCaster) {
  const prefix = `${trackingTurnKey()}|${caster.instanceId}|`
  return Object.entries(matchTracking.value.spellCasts || {}).filter(([key, state]) => key.startsWith(prefix) && (state.result === 'success' || state.result === 'fail')).length
}
function wizardSpellCastLimitReached(caster: GameMagicCaster) { return caster.kind === 'Wizard' && casterSpellCastLimitByLevel(caster) && wizardCompletedCastCount(caster) >= wizardEffectiveLevel(caster) }
function wizardCannotCast(rule: MatchGuidanceRule) {
  ...
  if (wizardSpellCastLimitReached(caster)) return true
...
function wizardCastingLockLabel(rule: MatchGuidanceRule) {
  ...
  if (wizardSpellCastLimitReached(caster)) return 'Cannot cast again this turn — spell casting limit reached'
  const state = matchTracking.value.wizardTurns?.[trackingTurnKey()]?.[caster.instanceId]
...
function toggleDeployedUnit(side: GameSide, instanceId: string, checked: boolean, internal = false) {
  ...
  const reserves = new Set(game.value.reservePlayerIds || [])
  // War Machines share a deployment stage, but each roster unit must retain
  // its own deployment state. Older match code toggled every War Machine when
  // any one of them was checked, which collapsed distinct units into one action.
...
function roundScore(side: GameSide) { if (!game.value) return 0; return Math.max(0, Number(game.value.roundScores?.[String(game.value.round)]?.[side] || 0)) }
function setRoundScore(side: GameSide, value: number) {
  if (!game.value || isReadOnly.value) return
  const score = Math.max(0, Math.floor(Number(value) || 0))
  const key = String(game.value.round)
  const current = { player: roundScore('player'), opponent: roundScore('opponent') }
  const previous = current[side]
  current[side] = score
  const roundScores = { ...(game.value.roundScores || {}), [key]: current }
  if (side === 'player') persist({ roundScores, playerScore: Math.max(0, game.value.playerScore - previous + score) })
  else persist({ roundScores, opponentScore: Math.max(0, game.value.opponentScore - previous + score) })
}
...
function rosterPoints(list: SavedArmyList) { const actual = Number(list.actualPoints || 0); return actual > 0 ? actual : (list.roster || []).reduce((sum, row) => sum + Number(row.totalPoints || 0), 0) }
function openRosterChange(side: GameSide) { if (isReadOnly.value) return; changeRosterSide.value = side; changeRosterId.value = side === 'player' ? String(game.value?.playerListId || '') : String(game.value?.opponentListId || '') }
function closeRosterChange() { changeRosterSide.value = null; changeRosterId.value = '' }
function pruneTrackingForFriendlyRoster(rows: BuilderRosterSelection[]) {
  const ids = new Set(rows.map((row) => row.instanceId))
  mutateTracking((next) => {
    next.joinedCharacters = Object.fromEntries(Object.entries(next.joinedCharacters).filter(([child, host]) => ids.has(child) && ids.has(host)))
    next.units = Object.fromEntries(Object.entries(next.units).filter(([id]) => ids.has(id)))
    next.turns = Object.fromEntries(Object.entries(next.turns).map(([key, group]) => [key, Object.fromEntries(Object.entries(group).filter(([id]) => ids.has(id)))]))
    next.chargeHistory = next.chargeHistory.filter((row) => ids.has(row.instanceId))
    next.combatHistory = next.combatHistory.filter((row) => ids.has(row.instanceId))
    next.spellCasts = {}; next.wizards = {}; next.wizardTurns = {}
  })
}
async function applyRosterChange() {
  if (!game.value || isReadOnly.value || !changeRosterSide.value || !changeRosterId.value) return
  const list = getSavedArmyList(changeRosterId.value); if (!list) return
  const rows = (list.roster || []).map((row) => ({ ...row }))
  if (changeRosterSide.value === 'player') {
    persist({ playerListId: list.id, playerListName: list.name, playerArmyName: list.armyName, playerArmyId: list.army, playerCompositionName: list.compositionName, playerCompositionRule: list.rule, playerOptions: [...(list.options || [])], playerRoster: rows, playerPoints: rosterPoints(list), magicSetup: [], deployedPlayerIds: [], reservePlayerIds: [] })
    pruneTrackingForFriendlyRoster(rows); magicCasters.value = []; combatProfiles.value = {}; await hydrateMagicSetup()
  } else {
    persist({ opponentListId: list.id, opponentListName: list.name, opponentArmyName: list.armyName, opponentArmyId: list.army, opponentCompositionName: list.compositionName, opponentCompositionRule: list.rule, opponentOptions: [...(list.options || [])], opponentRoster: rows, opponentPoints: rosterPoints(list), deployedOpponentIds: [] })
    turnGuidance.value = []; if (isBattleTurnPhase.value) void hydrateTurnGuidance()
  }
  closeRosterChange()
}
...
function casterRosterRow(caster: GameMagicCaster) { return playerRoster.value.find((row) => row.instanceId === caster.instanceId) || null }
function additionalSpellKnowledge(caster: GameMagicCaster) {
  const row = casterRosterRow(caster)
  if (!row) return 0
  const oneMore = /^(?:Learned Feng Shi Bo|Scrolls of Wei-jin|Spell Familiar|Silvery Wand|Tome of Furion)$/i
  return (row.magicItems || []).reduce((sum, item) => sum + (oneMore.test(String(item.name || '').replace(/\*+$/, '').trim()) ? Math.max(1, Number(item.count || 1)) : 0), 0)
}
function matchMagicSelectionLimit(caster: GameMagicCaster) { return caster.kind === 'Wizard' ? Math.max(1, Number(caster.level || 1) + additionalSpellKnowledge(caster)) : 0 }
function casterSpellCastLimitByLevel(caster: GameMagicCaster) { return Boolean(casterRosterRow(caster)?.magicItems?.some((item) => /^Scrolls of Wei-jin$/i.test(String(item.name || '').replace(/\*+$/, '').trim()))) }
function toggleMagicChoice(caster: GameMagicCaster, id: string, selected: boolean) { if (isReadOnly.value || caster.kind !== 'Wizard') return; const next = new Set(caster.selectedSpellIds); if (selected) { if (!next.has(id) && next.size >= matchMagicSelectionLimit(caster)) return; next.add(id) } else next.delete(id); caster.selectedSpellIds = [...next]; persistMagicSetup() }
function casterChoiceDisabled(caster: GameMagicCaster, id: string) { return !selectedMagicChoice(caster, id) && caster.selectedSpellIds.length >= matchMagicSelectionLimit(caster) }
...
<template>
  <main class="page game-match-page">
    ...
    <section v-if="game" class="game-match-shell">
      ...
      <div class="match-tab-scroll-shell phase-scroll-shell combined-phase-scroll-shell">
        ...
        <nav ref="phaseTabsRef" class="game-phase-tabs combined-phase-tabs" aria-label="Battle phases and current subphases">
          <template v-for="(item, phaseIndex) in gameWorkflow" :key="item.id">
            <button type="button" class="phase-parent-tab" :class="{ active: game.phaseIndex === phaseIndex, expanded: game.phaseIndex === phaseIndex }" :disabled="isReadOnly" @click="setPhase(phaseIndex)">{{ item.label }}</button>
            <template v-if="game.phaseIndex === phaseIndex">
              <template v-for="(subphase, stepIndex) in item.steps" :key="`${item.id}-${subphase.id}`">
                <span class="phase-subphase-separator" aria-hidden="true">·</span>
                <button type="button" class="phase-inline-step" :class="{ active: game.stepIndex === stepIndex }" :disabled="isReadOnly" @click="setStep(stepIndex)">{{ subphase.label }}</button>
              </template>
            </template>
          </template>
        </nav>
      ...
      <section v-if="phase && step" class="game-step-card card-surface">
        ...
        <div v-if="isSetupArmiesStep" class="game-setup-content">
          ...
          <OngoingBattleConditionsPanel v-if="showOngoingBattleConditions && ongoingBattleConditionDisplayRows.length" :rows="ongoingBattleConditionDisplayRows" />
          <section class="match-setup-summary-grid">
            <article class="match-roster-summary friendly"><p class="eyebrow">FRIENDLY GENERAL <button v-if="!isReadOnly" type="button" class="inline-link-button" @click="openRosterChange('player')">(Change Roster)</button></p><h3>{{ game.playerName }}</h3><strong>{{ game.playerListName }}</strong><p>{{ game.playerArmyName }}</p><dl><div><dt>Points</dt><dd>{{ playerActualPoints || game.points }} / {{ game.points }}</dd></div><div><dt>Composition</dt><dd>{{ playerCompositionName }}</dd></div><div><dt>Battle composition</dt><dd>{{ playerCompositionRule }}</dd></div></dl><div v-if="playerOptionLabels.length" class="match-option-chips"><span v-for="label in playerOptionLabels" :key="label">{{ label }}</span></div></article>
            <article class="match-roster-summary enemy"><p class="eyebrow">ENEMY GENERAL <button v-if="!isReadOnly" type="button" class="inline-link-button" @click="openRosterChange('opponent')">(Change Roster)</button></p><h3>{{ game.opponentName }}</h3><strong>{{ game.opponentListName || 'No enemy roster selected' }}</strong><p>{{ game.opponentArmyName || 'Opponent' }}</p><dl><div><dt>Points</dt><dd>{{ opponentActualPoints || game.opponentPoints || 0 }} / {{ game.points }}</dd></div><div><dt>Composition</dt><dd>{{ opponentCompositionName }}</dd></div><div><dt>Battle composition</dt><dd>{{ opponentCompositionRule }}</dd></div></dl><div v-if="opponentOptionLabels.length" class="match-option-chips"><span v-for="label in opponentOptionLabels" :key="label">{{ label }}</span></div></article>
          </section>
        ...
        <div v-else-if="isSetupSpellsStep" class="game-setup-content spell-setup-content">
          ...
          <template v-else>
            <details v-for="caster in magicCasters" :key="caster.instanceId" class="spell-caster-panel caster-collapse-panel">
              <summary class="spell-caster-heading"><div><span class="rule-kind-pill">{{ caster.kind }}<template v-if="caster.kind === 'Wizard'"> · Level {{ caster.level }}</template></span><h3>{{ caster.name }}</h3><p>{{ caster.selectedLore || 'No lore selected' }}</p></div><strong v-if="caster.kind === 'Wizard'">{{ caster.selectedSpellIds.length }} / {{ matchMagicSelectionLimit(caster) }} spells</strong></summary>
        ...
        <div v-else-if="isOverviewStep" class="game-overview-dashboard">
          ...
          <section class="overview-matchup card-inset"><div><p class="eyebrow">FRIENDLY <button v-if="!isReadOnly" type="button" class="inline-link-button" @click="openRosterChange('player')">(Change Roster)</button></p><h3>{{ game.playerName }}</h3><strong>{{ game.playerListName }}</strong><p>{{ game.playerArmyName }} · {{ game.playerPoints || playerActualPoints || game.points }} pts</p></div><span>—</span><div><p class="eyebrow">ENEMY <button v-if="!isReadOnly" type="button" class="inline-link-button" @click="openRosterChange('opponent')">(Change Roster)</button></p><h3>{{ game.opponentName }}</h3><strong>{{ game.opponentListName || 'No enemy roster' }}</strong><p>{{ game.opponentArmyName || 'Opponent' }} · {{ game.opponentPoints || opponentActualPoints || 0 }} pts</p></div></section>
        ...
        <section v-if="isBattleTurnPhase" class="turn-guidance-shell">
          ...
          <template v-else>
            ...
            <article v-if="isDeclareChargeStep" class="turn-guidance-panel declare-charge-panel card-inset"><div class="setup-section-heading"><div><p class="eyebrow">CHARGE SEQUENCE</p><h3>Eligible charging units</h3><small class="optional-check-hint">Joined characters travel with their unit and are not listed as separate chargers.</small></div></div><div v-if="declareChargeGuidance.length" class="declare-charge-sequence-list"><article v-for="(rule, index) in declareChargeGuidance" :key="`${rule.source}-${rule.unitRefs?.[0]?.instanceId || index}`" class="declare-resolve-charge-row" :class="{ complete: chargeHeld(rule.unitRefs?.[0]?.instanceId || '') || chargeInCombat(rule.unitRefs?.[0]?.instanceId || '') || chargeSuccessful(rule.unitRefs?.[0]?.instanceId || ''), required: rule.requiredCharge, held: chargeHeld(rule.unitRefs?.[0]?.instanceId || ''), inCombat: chargeInCombat(rule.unitRefs?.[0]?.instanceId || ''), successful: chargeSuccessful(rule.unitRefs?.[0]?.instanceId || '') }"><header><div><strong>{{ rule.unitRefs?.[0]?.name || rule.source }}</strong><small v-if="rule.requiredCharge" class="required-charge-label">MUST CHARGE IF POSSIBLE</small><div v-if="joinedCharactersForHost(rule.unitRefs?.[0]?.instanceId || '').length" class="joined-character-pills"><span v-for="character in joinedCharactersForHost(rule.unitRefs?.[0]?.instanceId || '')" :key="`charge-joined-${character.instanceId}`">{{ character.name }}</span></div></div><strong class="charge-roll-sequence">{{ rule.unitRefs?.[0]?.chargeRollNote || 'Charge Roll' }}</strong><span class="declare-charge-range"><small>MAX DECLARATION RANGE</small><strong>{{ rule.unitRefs?.[0]?.chargeRange || 'See profile' }}</strong><em v-if="rule.unitRefs?.[0]?.chargeRangeNote">{{ rule.unitRefs?.[0]?.chargeRangeNote }}</em></span></header><div class="charge-resolution-checks"><label><input type="checkbox" :checked="chargeHeld(rule.unitRefs?.[0]?.instanceId || '')" :disabled="isReadOnly || chargeInCombat(rule.unitRefs?.[0]?.instanceId || '') || chargeSuccessful(rule.unitRefs?.[0]?.instanceId || '')" @change="setChargeHeld(rule.unitRefs?.[0]?.instanceId || '', ($event.target as HTMLInputElement).checked)" /><span>Stay</span></label><label><input type="checkbox" :checked="chargeInCombat(rule.unitRefs?.[0]?.instanceId || '')" :disabled="isReadOnly || chargeHeld(rule.unitRefs?.[0]?.instanceId || '') || chargeSuccessful(rule.unitRefs?.[0]?.instanceId || '')" @change="setChargeInCombat(rule.unitRefs?.[0]?.instanceId || '', ($event.target as HTMLInputElement).checked)" /><span>In Combat</span></label><label><input type="checkbox" :checked="chargeSuccessful(rule.unitRefs?.[0]?.instanceId || '')" :disabled="isReadOnly || chargeHeld(rule.unitRefs?.[0]?.instanceId || '') || chargeInCombat(rule.unitRefs?.[0]?.instanceId || '')" @change="setChargeSuccessful(rule.unitRefs?.[0]?.instanceId || '', ($event.target as HTMLInputElement).checked)" /><span>Successful Charge</span></label></div><details v-if="chargeRelatedRules(rule).length" class="phase-rule-details charge-unit-rule-details"><summary>Rules that apply while resolving this charge</summary><article v-for="related in chargeRelatedRules(rule)" :key="`${related.label}-${related.path || ''}-${related.summary}`" class="charge-related-rule"><strong>{{ related.label }}</strong><div class="match-rule-copy"><p v-for="paragraph in ruleParagraphs(related.summary)" :key="paragraph">{{ paragraph }}</p></div><RouterLink v-if="related.path" :to="`/rules/read${related.path}`">Open rule</RouterLink></article></details></article></div><p v-else class="setup-inline-status">No friendly unit is available for a charge declaration in the current roster snapshot.</p></article>

            <article v-if="isEnemyDeclareChargeStep" class="turn-guidance-panel charge-reaction-panel card-inset">
              <div class="setup-section-heading"><div><p class="eyebrow">CHARGE REACTIONS</p><h3>Friendly units</h3><small class="optional-check-hint">Joined Characters use the reaction chosen for the unit they joined and are not listed separately.</small></div></div>
              <div v-if="enemyChargeReactionUnits.length" class="charge-reaction-unit-list">
                <article v-for="unit in enemyChargeReactionUnits" :key="`reaction-${unit.instanceId}`" class="charge-reaction-unit">
                  <header><div><strong>{{ unit.name }}</strong><div v-if="joinedCharactersForHost(unit.instanceId).length" class="joined-character-pills"><span v-for="character in joinedCharactersForHost(unit.instanceId)" :key="`reaction-joined-${character.instanceId}`">{{ character.name }}</span></div></div><small v-if="chargeReaction(unit.instanceId)" class="value-chip">{{ chargeReactionLabel(chargeReaction(unit.instanceId)) }}</small></header>
                  <div class="charge-reaction-options"><label><input type="checkbox" :checked="chargeReaction(unit.instanceId) === 'hold'" :disabled="isReadOnly" @change="setChargeReaction(unit.instanceId, 'hold', ($event.target as HTMLInputElement).checked)" /><span>Hold</span></label><label v-if="missileWeaponsForUnit(unit.instanceId).length"><input type="checkbox" :checked="chargeReaction(unit.instanceId) === 'stand-shoot'" :disabled="isReadOnly" @change="setChargeReaction(unit.instanceId, 'stand-shoot', ($event.target as HTMLInputElement).checked)" /><span>Stand &amp; Shoot</span></label><label><input type="checkbox" :checked="chargeReaction(unit.instanceId) === 'flee'" :disabled="isReadOnly" @change="setChargeReaction(unit.instanceId, 'flee', ($event.target as HTMLInputElement).checked)" /><span>Flee</span></label><label v-if="unitHasCounterCharge(unit.instanceId)"><input type="checkbox" :checked="chargeReaction(unit.instanceId) === 'counter-charge'" :disabled="isReadOnly" @change="setChargeReaction(unit.instanceId, 'counter-charge', ($event.target as HTMLInputElement).checked)" /><span>Counter Charge</span></label></div>
                  <section v-if="chargeReaction(unit.instanceId) === 'stand-shoot'" class="stand-shoot-resolution">
                    <div class="shooting-bs-summary"><span class="value-chip">BS {{ ballisticSkill(unit.instanceId) || '—' }}</span><span class="value-chip shooting-to-hit-chip">To Hit {{ shootingToHitLabel(unit.instanceId) }}</span><span class="value-chip">Stand &amp; Shoot −1</span></div>
                    <div class="shooting-weapon-list"><div v-for="weapon in missileWeaponsForUnit(unit.instanceId)" :key="`stand-shoot-${unit.instanceId}-${weapon.id}`" class="shooting-weapon-row"><strong>{{ weapon.name }}<small v-if="weapon.count > 1"> ×{{ weapon.count }}</small></strong><span>{{ weapon.range }}</span><span>S {{ weapon.strength }}</span><span>AP {{ weapon.ap }}</span><small v-if="weapon.rules.length">{{ weapon.rules.join(' · ') }}</small></div></div>
                    <div class="shooting-penalty-options"><label v-for="penalty in standShootSelectablePenalties" :key="`stand-shoot-penalty-${unit.instanceId}-${penalty.id}`" :class="{ selected: shootingPenaltyIds(unit.instanceId).has(penalty.id) }"><input type="checkbox" :checked="shootingPenaltyIds(unit.instanceId).has(penalty.id)" :disabled="isReadOnly" @change="toggleShootingPenalty(unit.instanceId, penalty.id, ($event.target as HTMLInputElement).checked)" /><span>{{ penalty.label }}</span><strong>{{ penalty.modifier }}</strong></label></div>
                  </section>
                  <details v-if="chargeReactionRules(unit.instanceId).length" class="phase-rule-details charge-unit-rule-details"><summary>Rules that apply to this charge reaction</summary><article v-for="rule in chargeReactionRules(unit.instanceId)" :key="`reaction-rule-${unit.instanceId}-${rule.label}-${rule.path || ''}`" class="charge-related-rule"><strong>{{ rule.label }}</strong><div class="match-rule-copy"><p v-for="paragraph in ruleParagraphs(rule.summary)" :key="paragraph">{{ paragraph }}</p></div><RouterLink v-if="rule.path" :to="`/rules/read${rule.path}`">Open rule</RouterLink></article></details>
                </article>
              </div><p v-else class="setup-inline-status">No friendly units are available to receive a charge.</p>
            </article>

            <article v-if="isCompulsoryMoveStep" class="turn-guidance-panel movement-unit-panel compulsory-move-panel card-inset"><div class="setup-section-heading"><div><p class="eyebrow">COMPULSORY MOVES</p><h3>Units that require compulsory movement</h3></div></div><div v-if="compulsoryMovementUnits.length" class="movement-unit-checklist"><label v-for="unit in compulsoryMovementUnits" :key="unit.instanceId" :class="{ complete: compulsoryMoved(unit.instanceId) }"><input type="checkbox" :checked="compulsoryMoved(unit.instanceId)" :disabled="isReadOnly" @change="setCompulsoryMoved(unit.instanceId, ($event.target as HTMLInputElement).checked)" /><span><strong>{{ unit.name }}</strong><small>{{ pursuedOffTable(unit.instanceId) ? 'Return as Reinforcements — Pursued Off-Table' : 'Compulsory movement resolved' }}</small></span></label></div><p v-else class="setup-inline-status">No friendly units currently require a tracked compulsory move.</p></article>
            ...
            <article v-if="isCombatFightStep" class="turn-guidance-panel combat-unit-panel combat-profile-panel card-inset">
              ...
              <div v-if="combatFightUnits.length" class="combat-fight-list"><article v-for="unit in combatFightUnits" :key="unit.instanceId" class="combat-fight-unit" :class="{ complete: combatUnitChecked(unit.instanceId) }"><label class="turn-action-check combat-fight-check"><input type="checkbox" :checked="combatUnitChecked(unit.instanceId)" :disabled="isReadOnly" @change="toggleCombatUnit(unit.instanceId, ($event.target as HTMLInputElement).checked)" /><span aria-hidden="true"></span></label><RouterLink class="combat-fight-unit-copy combat-fight-profile-area" :to="matchUnitProfileRoute(unit)"><div class="combat-unit-name-line"><strong>{{ unit.name }}</strong><span v-if="unitChargedThisTurn(unit.instanceId)" class="value-chip combat-charged-chip">Charged</span><span v-if="hasImpactHits(unit)" class="value-chip combat-impact-chip">Impact Hits</span></div><small>{{ combatRemainingLabel(unit) }}</small><small v-if="joinedHostId(unit.instanceId)" class="combat-joined-note">Joined to {{ joinedHostName(unit.instanceId) }}</small></RouterLink></article></div>
              ...
              <div v-if="foughtUnits.length" class="combat-result-unit-list">
                <article v-for="unit in foughtUnits" :key="`combat-result-${unit.instanceId}`" class="combat-result-unit compact-combat-resolution">
                  ...
                  <template v-else>
                    ...
                    <section v-if="combatDispositionFor(unit.instanceId) === 'lost'" class="break-test-resolution">
                      <label class="combat-loss-input"><span>Lost combat by</span><input type="number" min="0" max="20" :value="combatLostBy(unit.instanceId)" :disabled="isReadOnly" @change="setCombatLostBy(unit.instanceId, Number(($event.target as HTMLInputElement).value))" /></label>
                      <div class="break-leadership-summary"><span>Leadership <strong>{{ unitBreakBands(unit).leadership }}</strong></span><span>Modified Give Ground threshold <strong>{{ unitBreakBands(unit).modifiedThreshold }}</strong></span></div>
                      <div class="break-outcome-grid"><label v-for="result in breakResultOptions" :key="`${unit.instanceId}-${result.id}`" class="break-outcome-card" :class="{ selected: unitTurnState(unit.instanceId).breakResult === result.id }"><input type="checkbox" :checked="unitTurnState(unit.instanceId).breakResult === result.id" :disabled="isReadOnly" @change="setBreakResult(unit.instanceId, result.id, ($event.target as HTMLInputElement).checked)" /><span><strong>{{ result.label }}</strong><small v-if="result.id === 'give-ground'">{{ unitBreakBands(unit).giveGround }}</small><small v-else-if="result.id === 'fall-back'">{{ unitBreakBands(unit).fallBack }}</small><small v-else>{{ unitBreakBands(unit).breakAndFlee }}</small></span></label></div>
                      <label v-if="hasNotPaidToFight(unit)" class="break-outcome-card war-machine-abandon-card" :class="{ selected: persistentUnitState(unit.instanceId).warMachineAbandoned }"><input type="checkbox" :checked="persistentUnitState(unit.instanceId).warMachineAbandoned" :disabled="isReadOnly" @change="setWarMachineAbandoned(unit, ($event.target as HTMLInputElement).checked)" /><span><strong>We Aren't Paid to Fight</strong><small>If this War Machine Breaks and flees from combat, its crew abandons it. Treat the model as destroyed and remove it from play.</small></span></label>
                    </section>
                    <fieldset v-else-if="combatDispositionFor(unit.instanceId) === 'won'" class="winner-follow-up-panel"><legend>Winner follow up</legend><div class="follow-up-outcome-grid"><label v-for="result in followUpOptions" :key="`${unit.instanceId}-follow-${result.id}`" class="break-outcome-card follow-up-outcome-card" :class="{ selected: unitTurnState(unit.instanceId).followUpResult === result.id }"><input type="checkbox" :checked="unitTurnState(unit.instanceId).followUpResult === result.id" :disabled="isReadOnly" @change="setFollowUpResult(unit.instanceId, result.id, ($event.target as HTMLInputElement).checked)" /><span><strong>{{ result.label }}</strong><small>{{ followUpDetail(unit, result.id) }}</small></span></label><label class="break-outcome-card follow-up-outcome-card pursued-off-table-card" :class="{ selected: pursuedOffTable(unit.instanceId) }"><input type="checkbox" :checked="pursuedOffTable(unit.instanceId)" :disabled="isReadOnly" @change="setPursuedOffTable(unit.instanceId, ($event.target as HTMLInputElement).checked)" /><span><strong>Pursued Off-Table</strong><small>The unit returns as Reinforcements in its controlling player's next Compulsory Moves subphase, as close as possible to the point it left the battlefield.</small></span></label></div></fieldset>
        ...
        <section v-if="isEndScoreStep" class="end-round-score-content" aria-label="Round score calculation"><div class="setup-section-heading"><div><p class="eyebrow">END OF ROUND · STEP 2 OF 2</p><h3>Round &amp; Score Calculation</h3></div></div><p>Enter the points scored by each player this round. Old.dex adds them to each running match total.</p><div class="game-score-board end-round-score-board"><label class="game-score-side"><small>{{ game.playerListName }}</small><span>Round {{ game.round }} score</span><input type="number" min="0" step="1" :value="roundScore('player')" :disabled="isReadOnly" @input="setRoundScore('player', Number(($event.target as HTMLInputElement).value))" /><strong>Running total: {{ game.playerScore }}</strong></label><span class="game-score-divider">—</span><label class="game-score-side"><small>{{ game.opponentListName || game.opponentName }}</small><span>Round {{ game.round }} score</span><input type="number" min="0" step="1" :value="roundScore('opponent')" :disabled="isReadOnly" @input="setRoundScore('opponent', Number(($event.target as HTMLInputElement).value))" /><strong>Running total: {{ game.opponentScore }}</strong></label></div></section>
      ...
      <div v-if="changeRosterSide" class="match-modal-backdrop" role="presentation" @click.self="closeRosterChange">
        <section class="match-roster-change-modal card-surface" role="dialog" aria-modal="true" aria-labelledby="change-roster-title">
          <p class="eyebrow">MATCH ROSTER</p><h2 id="change-roster-title">Change {{ changeRosterSide === 'player' ? 'Friendly' : 'Enemy' }} Roster</h2>
          <p>Choose a saved roster. The match snapshot and related roster-specific tracking will update to the selected list.</p>
          <label><span>Roster</span><select v-model="changeRosterId"><option value="">Select a roster</option><option v-for="list in rosterChangeChoices" :key="list.id" :value="list.id">{{ list.name }} — {{ list.armyName }} — {{ rosterPoints(list) }} pts</option></select></label>
          <div class="match-modal-actions"><button type="button" class="secondary-button" @click="closeRosterChange">Cancel</button><button type="button" class="primary-button" :disabled="!changeRosterId" @click="applyRosterChange">Change Roster</button></div>
        </section>
      </div>
