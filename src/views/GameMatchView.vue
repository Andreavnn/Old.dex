...
function setChargeHeld(instanceId: string, checked: boolean) {
  patchUnitTurnState(instanceId, { chargeHeld: checked, ...(checked ? { chargeDeclared: false, chargeSuccessful: false } : {}) })
}
  ...
  <main class="page game-match-page">
    ...
    <section v-if="game" class="game-match-shell">
      ...
      <section v-if="phase && step" class="game-step-card card-surface">
            ...
            <article v-if="isCombatResultStep" class="turn-guidance-panel combat-result-tracker combined-combat-resolution card-inset">
              ...
              <div v-if="foughtUnits.length" class="combat-result-unit-list">
                <article v-for="unit in foughtUnits" :key="`combat-result-${unit.instanceId}`" class="combat-result-unit compact-combat-resolution">
                  ...
                  <template v-if="joinedHostId(unit.instanceId)"><p class="combat-result-host-note">Wounds are tracked on this Character. Combat Result, Break Test and follow-up are resolved with {{ joinedHostName(unit.instanceId) }}.</p></template>
                  <template v-else>
        ...
        <details class="game-step-note-panel"><summary>Note</summary><label class="game-step-notes"><textarea v-model="notes" :readonly="isReadOnly" rows="3" placeholder="Record targets, results, effects, or table notes for this step." @blur="saveNotes"></textarea></label></details>
