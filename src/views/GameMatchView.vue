    ...
    <section v-if="game" class="game-match-shell">
      ...
      <section v-if="phase && step" class="game-step-card card-surface">
        ...
        <section v-if="isBattleTurnPhase" class="turn-guidance-shell">
          <MatchTipPanel v-if="tipsVisible && battleStepTip && !isAnyDeclareChargeStep && !isBreakFollowStep" :title="`Tip — ${step.label}`" :text="battleStepTip" :collapsed="currentTipCollapsed" :rule-to="`/rules/read${battleStepRulePath}`" rule-label="Open phase rules" @toggle="setCurrentTipCollapsed" />
