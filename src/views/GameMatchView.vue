...
function scrollTabs(target: 'phase', direction: -1 | 1) {
...
function wizardCannotCastReason(rule: MatchGuidanceRule) {
  ...
  if (wizardSpellCastLimitReached(caster)) return 'Spell casting limit reached for this turn'
              ...
              <div v-if="foughtUnits.length" class="combat-result-unit-list">
                <article v-for="unit in foughtUnits" :key="`combat-result-${unit.instanceId}`" class="combat-result-unit compact-combat-resolution">
                  ...
                  <template v-else>
                    ...
                    <fieldset v-else-if="combatDispositionFor(unit.instanceId) === 'won'" class="winner-follow-up-panel"><legend>Winner follow up</legend><div class="follow-up-outcome-grid"><label v-for="result in followUpOptions" :key="`${unit.instanceId}-follow-${result.id}`" class="break-outcome-card follow-up-outcome-card" :class="{ selected: unitTurnState(unit.instanceId).followUpResult === result.id }"><input type="checkbox" :checked="unitTurnState(unit.instanceId).followUpResult === result.id" :disabled="isReadOnly" @change="setFollowUpResult(unit.instanceId, result.id, ($event.target as HTMLInputElement).checked)" /><span><strong>{{ result.label }}</strong><small>{{ followUpDetail(unit, result.id) }}</small></span></label></div><label class="break-outcome-card pursued-off-table-card" :class="{ selected: pursuedOffTable(unit.instanceId) }"><input type="checkbox" :checked="pursuedOffTable(unit.instanceId)" :disabled="isReadOnly" @change="setPursuedOffTable(unit.instanceId, ($event.target as HTMLInputElement).checked)" /><span><strong>Pursued Off-Table</strong><small>May be recorded in addition to the selected pursuit/overrun result. The unit returns as Reinforcements in its controlling player's next Compulsory Moves subphase, as close as possible to the point it left the battlefield.</small></span></label></fieldset>
