...
<template>
  <article class="old-rule-card static-rule-card" :class="`tone-${rule.tone}`">
    <div class="old-rule-summary static-rule-summary">
      <span class="old-rule-pill-row"><span class="old-rule-phase">{{ rule.timing }}</span></span>
      <span class="old-rule-title-row"><RuleToneIcon :tone="rule.tone" :label="`${rule.timing} ${rule.name}`" /><span class="old-rule-name">{{ rule.name }}</span></span>
    </div>
    <div class="old-rule-body">
      ...
      <div class="old-rule-keywords" aria-label="Related rules">
        <span class="rule-kind-pill">{{ kindLabel || 'Special Rule' }}</span>
