...
<template>
  <nav class="match-general-bar card-surface" aria-label="General's Bar">
    <RouterLink class="match-general-tool" :class="{ active: props.active === 'reference' }" :to="props.active === 'reference' ? `/games/${props.game.id}` : `/games/${props.game.id}/reference-sheet`">Reference Sheet</RouterLink>
    <RouterLink class="match-general-tool" :class="{ active: props.active === 'charts' }" :to="props.active === 'charts' ? `/games/${props.game.id}` : `/games/${props.game.id}/battle-charts`">Battle Charts</RouterLink>
    <RouterLink class="match-general-tool" :class="{ active: props.active === 'roster' }" :to="props.active === 'roster' ? `/games/${props.game.id}` : `/games/${props.game.id}/roster`">Roster</RouterLink>
  ...
  <Teleport to="body">
    <div v-if="exportOpen" class="match-export-backdrop" role="presentation" @click.self="closeExport">
      <section class="match-export-modal card-surface" role="dialog" aria-modal="true" aria-labelledby="match-export-title">
