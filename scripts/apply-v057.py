...
CSS_MARKER = "/* Old.dex v0.57 UI cleanup */"
CSS_BLOCK = r'''
/* Old.dex v0.57 UI cleanup */
.discord-button-icon {
  width: 1.05em;
  height: 1.05em;
  flex: 0 0 auto;
...
def patch_unit_view(repo: Path) -> list[str]:
    ...
    if remaining_override_mutation:
        die(
            ...
            "or weapon modifiers before applying v0.57."
        )
...
def ensure_no_github_artifacts(repo: Path) -> None:
    for pattern in ("ODX_V0*_VERIFICATION*", "ODX-V0*-VERIFICATION*"):
        for path in repo.rglob(pattern):
            if path.is_file():
                path.unlink()
                print(f"[Old.dex v0.57] removed obsolete verification artifact: {path.relative_to(repo)}")
    ...
    if workflow_dir.exists():
        for path in workflow_dir.glob("*"):
            ...
            if (
                ...
                path.unlink()
                print(f"[Old.dex v0.57] removed obsolete temporary workflow: {path.relative_to(repo)}")

def main() -> None:
    parser = argparse.ArgumentParser(
        description="Apply Old.dex v0.57 canonical profile foundation rebuild."
    )
    parser.add_argument(
        "repo",
        nargs="?",
    ...
    for rel in unique:
        print(f"  - {rel}")
    print(
        "[Old.dex v0.57] No GitHub workflow, ODX verification marker, "
