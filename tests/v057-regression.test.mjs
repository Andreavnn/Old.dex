...
test('v0.57 starts the persistent Games workflow from saved lists', () => {
  ...
  for (const phase of ['Setup','Overview','Deployment','Strategy','Movement','Shooting','Combat']) assert.match(gameService, new RegExp(`label: '${phase}'`))
  assert.match(gameService, /label: 'End(?: of Round)?'/)
